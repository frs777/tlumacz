...
| **BackendManager** | `backend_manager.py` | Centralny zarządca 3 backendów, sygnały Qt |
| **ServerManager** | `qt_gui/worker.py` | Zarządca llama.cpp, maszyna stanów |
| **FastAPIServerManager** | `fastapi_manager.py` | Zarządca FastAPI + Transformers |
...
├── test_backend_manager.py # Testy BackendManager (25 testów)
...
| Pole | Typ | Opis |
|------|-----|------|
| `backend_type` | string | Typ backendu: `"llama"`, `"cloud"`, lub `"fastapi"` |
...
| `fastapi_model_path` | string | Ścieżka do katalogu modelu dla FastAPI |
