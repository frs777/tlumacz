...
**TranslateGemma wymaga backendu FastAPI + Transformers.** 

llama.cpp **NIE obsługuje** specjalnego formatu TranslateGemma z kodami języków (`source_lang_code`, `target_lang_code`). Próba użycia TranslateGemma z llama.cpp daje jakość jak zwykły model Gemma 3 (bez specjalnego formatu).
...
#### 2. **Specjalny format wiadomości z kodami języków**
...
- `nl` — holenderski
- `ru` — rosyjski
- `auto` — automatyczna detekcja języka źródłowego

**W Tłumaczu:** Wybierz backend **"FastAPI (TranslateGemma)"** w QComboBox "Typ serwera".
...
#### 1. Pobierz model z HuggingFace

```bash
# Z Hugging Face (wymaga zatwierdzenia od Google)
huggingface-cli download google/translategemma-4b-it \
  --local-dir ~/Modele/translategemma-4b-it
```

**Uwaga:** Model wymaga zatwierdzenia licencji od Google na HuggingFace.

#### 2. Zainstaluj zależności FastAPI

```bash
pip install -r requirements-fastapi.txt
```

Zależności: fastapi, uvicorn, transformers, torch, accelerate, safetensors

#### 3. Skonfiguruj Tłumacz

**Zakładka „API i serwer":**

| Pole | Wartość |
|------|---------|
| **Typ serwera** | `FastAPI (TranslateGemma)` |
| **Model** | `/home/user/Modele/translategemma-4b-it` (ścieżka do katalogu) |
| **Port** | `17680` (domyślny dla FastAPI) |
...
TranslateGemma wymaga backendu FastAPI + Transformers. W Tłumaczu:

- Wybierz **"FastAPI (TranslateGemma)"** z QComboBox "Typ serwera"
- Wskaż ścieżkę do **katalogu** modelu (nie pliku GGUF)
- FastAPI server automatycznie ładuje model z Transformers
...
### TranslateGemma (FastAPI)

```json
{
  "backend_type": "fastapi",
  "base_url": "http://127.0.0.1:17680/v1",
  "api_key": "ollama",
  "model": "local",
  "fastapi_model_path": "/home/user/Modele/translategemma-4b-it",
  "fastapi_port": 17680,
  "fastapi_device": "auto",
...
### TranslateGemma (llama.cpp — BEZ specjalnego formatu)

**Uwaga:** llama.cpp NIE obsługuje specjalnego formatu TranslateGemma. Model działa jak zwykły Gemma 3.
