...
W zakładce **Pomoc** znajduje się przycisk **O programie**. Okno pokazuje nazwę programu, aktualną wersję, podstawowy opis oraz licencję. Treść jest dostosowana do wybranego języka Pomocy (PL/EN).

**Ważne przy aktualizacji wersji:** numer wyświetlany w oknie nie jest wpisywany bezpośrednio w `main_window.py`. Należy zmienić `APP_VERSION` w `tlumacz/version.py` oraz wersję projektu w `pyproject.toml`, a następnie zaktualizować `QWEN.md` i wpisy wydania w dokumentacji. Test `test_about_dialog_shows_version_and_basic_info` chroni widoczność numeru wersji w GUI.
