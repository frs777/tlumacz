...
class TranslatorConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not base:
            # Sprawdź format "wykryj do X" (np. "wykryj do pl")
            if self.target_language.startswith("wykryj do "):
                target_lang = self.target_language.split("wykryj do ")[1].upper()
                base = (
                    f"You are a professional translator. "
                    f"Detect the source language and translate ALL text to {target_lang}. "
                    ...
                    f"Pay attention to grammar, especially inflection by person and grammatical gender."
                )
            else:
                base = (
                    f"You are a professional translator. "
