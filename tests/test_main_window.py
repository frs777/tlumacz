...
def test_about_dialog_shows_version_and_basic_info(qapp, config_home, monkeypatch):
    from PySide6.QtWidgets import QMessageBox

    from tlumacz.qt_gui.main_window import MainWindow
    from tlumacz.version import APP_VERSION

    window = MainWindow(server=None)
    shown = []
    monkeypatch.setattr(
        QMessageBox, "about", lambda *args, **kwargs: shown.append((args, kwargs))
    )

    window.about_btn.click()

    assert shown
    assert APP_VERSION in shown[0][0][-1]
    assert "tłumaczenia" in shown[0][0][-1].lower()
    assert window.about_btn.text() == "O programie"

    window.help_language.setCurrentIndex(1)
    assert window.about_btn.text() == "About"
    window.about_btn.click()
    assert APP_VERSION in shown[-1][0][-1]
    window.close()
