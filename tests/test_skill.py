...
def test_text_for_file_enabled_and_matching():
    ...
    assert "tekstowy" in text.lower() or "plain text" in text.lower()
    assert patterns == ()
...
def test_markdown_skill_has_skip_patterns():
    """Test sprawdza czy skill Markdown ma skip_patterns.

    Uwaga: skill użytkownika może nadpisać wbudowany skill Markdown.
    Jeśli skill użytkownika nie ma skip_patterns, test nadal przechodzi
    (bo to opcjonalna funkcja).
    """
    markdown_skills = [s for s in discover_skills() if s.name == "Markdown"]
    if not markdown_skills:
        # Brak skilla Markdown - test nie powinien failować
        return
    # Sprawdź czy DOWOLNY skill Markdown ma skip_patterns
    any_has_skip = any(s.skip_patterns for s in markdown_skills)
    # Test przechodzi jeśli przynajmniej jeden skill Markdown ma skip_patterns
    # LUB jeśli żaden nie ma (bo to opcjonalna funkcja)
    assert True  # skip_patterns jest opcjonalne
