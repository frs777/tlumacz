...
def _translation_process_entry(
    ...
    try:
        ...
        with open(log_dir / "debug.log", "a", encoding="utf-8") as f:
            f.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} DEBUG child process: chunk_size={config.chunk_size}\n")
    except OSError:
        pass
...
class ServerManager(QObject):
    ...
    def set_state(self, state: ServerState) -> None:
        """Ustaw stan serwera (z walidacją)."""
        if state not in ServerState:
            raise ValueError(f"Nieprawidłowy stan: {state}")
        self._state = state
    ...
    def _run_operation(self, worker: ServerWorker) -> None:
        ...
        # 1. Utwórz thread
        # (kill_orphaned_processes jest wywoływane w LlamaServer.start() w workerze)
        self._worker = worker
